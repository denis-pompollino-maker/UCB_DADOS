...
import sqlite3
import io  # Adicionado para suportar a geração do Excel em memória
...
with st.sidebar:
    ...
    if uploaded_file:
        menu = st.radio("NAVEGAÇÃO", [
            ...
            "📋 NIPPO COORDENADORES",
            "🛠️ ADMIN & BACKUPS"      # Nova aba inserida para gestão de backups
        ])
...
if uploaded_file:
    ...
    # ABA: ADMIN & BACKUPS
    # =========================================================
    elif menu == "🛠️ ADMIN & BACKUPS":
        st.markdown("## 🛠️ Painel de Administração e Backups")
        st.info("Central de segurança: exporte os dados do servidor em Excel ou faça o download integral do banco de dados SQLite para backup local.")

        col_bkp1, col_bkp2 = st.columns(2)

        with col_bkp1:
            st.markdown("### 📊 Exportação para Excel")
            st.markdown("Gera um arquivo `.xlsx` contendo todas as abas de dados registrados no sistema.")
            
            if st.button("🔄 Gerar Arquivo Excel com Todos os Registros", use_container_width=True):
                # Conecta e extrai todas as tabelas
                conn = sqlite3.connect('reportes_turno.db')
                df_rep = pd.read_sql_query("SELECT * FROM reportes", conn)
                df_sem = pd.read_sql_query("SELECT * FROM analises_semanais", conn)
                df_nip = pd.read_sql_query("SELECT * FROM nippo_coordenadores", conn)
                conn.close()

                # Cria o arquivo Excel em memória
                buffer = io.BytesIO()
                with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
                    df_rep.to_excel(writer, sheet_name='Reportes Diarios', index=False)
                    df_sem.to_excel(writer, sheet_name='Analises Semanais', index=False)
                    df_nip.to_excel(writer, sheet_name='Nippo Coordenadores', index=False)

                st.download_button(
                    label="📥 BAIXAR DADOS EM EXCEL (.xlsx)",
                    data=buffer.getvalue(),
                    file_name=f"Backup_Industrial_Hub_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
        ...
        with col_bkp2:
            st.markdown("### 💾 Backup Raw do Banco de Dados")
            st.markdown("Baixe o arquivo `.db` original para restaurar em caso de falha do servidor ou migração.")

            try:
                with open("reportes_turno.db", "rb") as fp:
                    st.download_button(
                        label="📥 BAIXAR BANCO DE DADOS (.db)",
                        data=fp,
                        file_name=f"reportes_turno_bkp_{datetime.now().strftime('%Y%m%d_%H%M')}.db",
                        mime="application/octet-stream",
                        use_container_width=True
                    )
            except FileNotFoundError:
                st.error("Banco de dados ainda não foi criado no servidor.")
