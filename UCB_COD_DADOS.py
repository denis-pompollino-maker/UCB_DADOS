...
# --- ESTILIZAÇÃO CSS LIGHT MODE ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;900&display=swap');
    * { font-family: 'Inter', sans-serif; }
    
    /* Fundo da Aplicação e Texto Global */
    ...
    [data-testid="stSidebar"] .stRadio div[role="radiogroup"] > label {
        ...
        margin-bottom: 5px !important; color: #1e293b !important; cursor: pointer;
    }
    ...
    .metric-value { color: #0f172a; font-size: 1.3rem; font-weight: 900; line-height: 1; }

    /* Calendário */
    .calendar-day-name { text-align: center; font-weight: 900; color: #059669; font-size: 0.8rem; padding-bottom: 5px; }
    ...
    .day-status { font-size: 0.75rem; font-weight: 600; color: #1e293b; text-align: right; }

    /* Ranking e Feedback */
    .feedback-box { padding: 12px; border-radius: 8px; margin-bottom: 10px; text-align: center; font-weight: 700; font-size: 0.9rem; }

    /* 5 Porquês */
    .five-why-box { border: 2px solid #000; padding: 15px; background: #fff; color: #000; margin-top: 15px; }
    .five-why-line { border-bottom: 1px dotted #333; padding: 10px 0; font-size: 0.9rem; }

    /* Reporte Diário Header */
    .section-header {
        background: #f1f5f9; padding: 10px; border-radius: 5px;
        color: #059669; font-weight: 800; text-transform: uppercase;
        margin-top: 20px; border-left: 5px solid #059669; font-size: 0.9rem;
    }
    
    /* Ajuste de cor de títulos e textos Streamlit */
...
# --- SIDEBAR ---
with st.sidebar:
    st.markdown("<h1 style='color:#059669;'>🏭 ANALYTICS HUB</h1>", unsafe_allow_html=True)
...
if uploaded_file:
    ...
    def mini_gauge(label, value, color, target, height=150):
        fig = go.Figure(go.Indicator(
            ...
            number={'suffix': "%", 'font': {'size': 18, 'color': 'black'}},
            title={'text': label, 'font': {'size': 12, 'color': 'black'}},
            gauge={'axis': {'range': [0, 100], 'tickcolor': "black"}, 'bar': {'color': color},
                   'threshold': {'line': {'color': "black", 'width': 2}, 'value': target}}
        ))
        fig.update_layout(height=height, margin=dict(l=10, r=10, t=30, b=10), paper_bgcolor='rgba(0,0,0,0)', font={'color': "black"})
        return fig
    ...
    if menu == "📋 REPORTE DIÁRIO":
        ...
        st.markdown(f"""
            <div class="metric-container">
                <div class="metric-card"><div class="metric-title">Movimentação Mês (Meta 90%)</div><div class="metric-value">{mov_acum_mes:.1f}%</div><div style="font-size:0.6rem; color:{'#059669' if gap_mov>=0 else '#e11d48'}">{gap_mov:+.1f}% vs meta</div></div>
                <div class="metric-card"><div class="metric-title">Loss Mês (Meta 2,5%)</div><div class="metric-value" style="color:#e11d48">{loss_acum_mes:.1f}%</div><div style="font-size:0.6rem; color:{'#059669' if gap_loss<=0 else '#e11d48'}">{gap_loss:+.1f}% vs meta</div></div>
            ...
            <div class="metric-container">
                <div class="metric-card"><div class="metric-title">Meta Acumulada (Até {data_ref_reporte.strftime('%d/%m')})</div><div class="metric-value" style="color:#2563eb">{meta_dinamica_hoje:,.0f}</div></div>
    ...
    # =========================================================
        ...
        with col2: st.plotly_chart(mini_gauge("Loss (%)", ((df_f['Machine Counter'].sum()-df_f['Peças Estoque - Ajuste'].sum())/df_f['Machine Counter'].sum()*100 if df_f['Machine Counter'].sum()>0 else 0), "#e11d48", 2.5, 280), use_container_width=True)
    ...
    # =========================================================
        ...
        fig2 = px.bar(df_s_f.groupby('Problema')['QTD'].sum().sort_values().tail(10), orientation='h', title="Frequência (Qtd)", color_discrete_sequence=['#2563eb'])
    ...
    # =========================================================
        ...
        days = list(calendar.Calendar(0).itermonthdays(2024, m_idx)) # Ajuste o ano conforme necessário
        ...
        for d in days:
            if d == 0: html_grid += '<div></div>'
                ...
                cor = "#10b981" if mov > 85 else "#fbbf24" if mov > 0 else "#f1f5f9"
                text_color = "white" if mov > 0 else "#64748b"
                html_grid += f'<div class="day-card" style="background:{cor}"><span class="day-number" style="color:{text_color}">{d}</span><div class="day-status" style="color:{text_color}">{mov:.1f}%</div></div>'
    ...
    # =========================================================
        ...
        df_b = df_order[df_order['Máquina'] == maq_b]
        df_sb = df_stops[df_stops['Máquina'] == maq_b]

        st.markdown(f"""<div style="text-align:center; border-bottom:3px solid #059669; padding-bottom:10px; margin-bottom:15px;">
            <h1 style="color:black; margin:0;">RELATÓRIO SEMANAL - MÁQUINA {maq_b}</h1>
            <p style="color:#64748b; font-size:1rem;">Análise de Performance e Causa Raiz</p></div>""", unsafe_allow_html=True)
        ...
        with v3: st.markdown(f'<div class="metric-card" style="height:150px;"><div class="metric-title">Peças Enviadas</div><div class="metric-value" style="font-size:1.8rem;">{df_b["Peças Estoque - Ajuste"].sum():,.0f}</div></div>', unsafe_allow_html=True)

        st.markdown(f"""
            <div class="five-why-box">
                <div style="font-weight:900; font-size:1.1rem; border-bottom:2px solid #000; margin-bottom:10px;">ANÁLISE DE CAUSA RAIZ - 5 PORQUÊS</div>
